    ...
    <section id="stile" className="py-20 bg-[#0b0b0e] border-b border-slate-800/80">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((s, idx) => (
            <div key={idx} className="bg-[#121216] border border-slate-800/80 rounded-3xl p-7 hover:border-[#c9a84c]/50 transition-all shadow-lg flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                  <h3 className="font-outfit font-bold text-xl text-white leading-snug">{s.title}</h3>
                  <span className="font-outfit font-extrabold text-[#c9a84c] text-lg shrink-0 ml-2">{s.price}</span>
                </div>

                <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest block">Abrechnung: {s.time}</span>
              ...
              <button
                ...
                className="w-full bg-[#c9a84c] hover:bg-[#b8973b] text-slate-950 font-bold text-xs py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 uppercase tracking-wider cursor-pointer"
              >
